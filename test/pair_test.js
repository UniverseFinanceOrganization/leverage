...
describe("Pair-Test", function() {
    ...
    let interestModel, oracle, config, lendVault, pairVault, vault;
    ...
    it("11、transfer token", async function() {
        ...
        await looks.connect(looksAccount).transfer(OPERATOR_ADDRESS, ethers.utils.parseUnits("10000000.0", 18));
    });
    ...
    it("13、open position", async function() {
            ...
            await pairVault.connect(operator).openPosition(0, VAULT_ADDRESS, token0Amount, token1Amount, token0Debt, token1Debt);
            positionId = 1;
    });
